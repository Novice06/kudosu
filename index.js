...
const PARTNER_URL = "https://inf-test.onrender.com"; // URL du bot partenaire
...
async function fillTopHalf(solvedValues) {
    try {
        ...
        // On ne remplit que les lignes 0 à 3 (premières 4 lignes)
        for (let i = 0; i < 36; i++) { // 4 lignes * 9 colonnes = 36 cellules
...
async function solveOneSudoku(roundNumber) {
    ...
    try {
        ...
        // Attendre que le partenaire termine
...
app.listen(PORT, () => {
    console.log(`🚀 Sudoku Solver Bot 1 (Top Half) running on port ${PORT}`);
