...
const PARTNER_URL = "https://inf-test.onrender.com"; // URL du bot bas
...
app.get("/", (req, res) => {
    res.json({
        message: "Sudoku Solver Bot Haut API is running",
        endpoints: {
...
app.post("/start-sudoku", async (req, res) => {
    ...
    try {
        isProcessing = true;
        solvedCount = 0;
        console.log("🚀 Démarrage du solveur Sudoku (Bot Haut)...");
...
async function fillMyPart(solvedValues) {
    try {
        ...
        // Remplir seulement les lignes 1 à 4 (indices 0-35)
        for (let i = 0; i < 36; i++) {
...
app.listen(PORT, () => {
    console.log(`🚀 Sudoku Solver Bot Haut running on port ${PORT}`);
});
